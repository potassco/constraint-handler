...
from flat_ch.core.domain import UserInput, FlatFact
...
class Flattener:
    def __init__(self, registry:PythonRegistry, constant_folding: bool = False, ):
    ...
    def flatten(self, clingo_term):
        ...
        if node_type not in STATEMENT_SCHEMAS:
            raise KeyError(
                f"No InputSchema registered for UserInput variant: '{node_type.name}'"
            )
    ...
    def _process_declaration(self, node_type, term_name, term_args):
        ...
        if len(term_args) != len(schema_layout):
            raise ValueError(
                f"Arity Mismatch: Node '{term_name}' expected {len(schema_layout)} "
                f"arguments, got {len(term_args)}."
            )
    ...
    def _flatten_expression(self, clingo_term) -> int:
        ...
        try:
            ...
            raise TypeError(
                f"Unrecognized expression: '{term_name}'. "
            ) from e

        if node_type not in self.expression_handlers:
            raise TypeError(
                f"No handler registered for expression type: '{node_type.name}'."
            )
    ...
    def _intern_value_expr(self, type_id: Type, value_symbol, pure_value) -> int:
        ...
        if expr_id is None:
            ...
            self.emitted_facts.append(
                Function(self._fact_expr_val, [Number(expr_id), Number(type_id.value), value_symbol])
            )
    ...
    def _parse_variable(self, clingo_term) -> int:
        ...
        if expr_id is None:
            ...
            self.structural_cache[cache_key] = expr_id
            self.emitted_facts.append(
                Function(self._fact_expr_var, [Number(expr_id), variable_symbol])
            )
        return expr_id
    ...
    def _parse_pair(self, clingo_term) -> int:
        ...
        if expr_id is None:
            ...
            self.structural_cache[cache_key] = expr_id
            self.emitted_facts.append(
                Function(self._fact_expr, [Number(expr_id), Function("pair", [])])
            )
            self.emitted_facts.append(
                Function(self._fact_pair, [Number(expr_id), Number(key_expr_id), Number(value_expr_id)])
            )
    ...
    def _parse_operation(self, clingo_term) -> int:
        ...
        if self.constant_folding:
            ...
            if is_pure:
                try:
                    if operator == Operator.PYTHON:
                        result_type, result_value = self._registry.execute_current_directly(
                            constant_args, argument_ids
                        )
                    else:
                    ...
                    print(f"Warning: Constant folding failed for operation '{operator.name}' with arguments {constant_args}. Falling back to standard flattening.")
                    pass
        ...
        if expr_id is None:
            ...
            if arity_count == 1 and (allowed & Arity.UNARY):
                self.emitted_facts.append(Function(f"op_{operator_name}", [eid_num, Number(argument_ids[0])]))

            elif arity_count == 2 and (allowed & Arity.BINARY):
                self.emitted_facts.append(Function(f"op_{operator_name}", [eid_num, Number(argument_ids[0]), Number(argument_ids[1])]))

            elif arity_count == 3 and (allowed & Arity.TERNARY):
                self.emitted_facts.append(Function(f"op_{operator_name}", [eid_num, Number(argument_ids[0]), Number(argument_ids[1]), Number(argument_ids[2])]))

            elif allowed & Arity.VARIADIC:
                self.emitted_facts.append(Function(f"op_{operator_name}_variadic", [eid_num, Number(arity_count)]))
                
                for position, arg_id in enumerate(argument_ids, start=1):
                    self.emitted_facts.append(Function(f"op_{operator_name}_arg", [eid_num, Number(position), Number(arg_id)]))
