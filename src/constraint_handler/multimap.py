...
def evaluate_operator(o, args, apply_operator=None) -> result.EvalResult:
    match o:
        ...
        case operators.MultimapOperator.find:
            assert len(args) == 2
            return result.EvalResult(args[1][args[0]] if args[0] in args[1] else frozenset(), NO_ERRORS)
        case operators.MultimapOperator.find2:
            assert len(args) == 3
            return result.EvalResult(args[1] in args[0] and args[2] in args[0][args[1]], NO_ERRORS)
        ...
        case operators.MultimapOperator.multimap_fold_i:
            ...
            return result.EvalResult(fold_i(step, args[1], args[2]), tuple(fold_errors))
        case operators.MultimapOperator.multimap_make:
            return result.EvalResult(Multimap.from_pairs(args), NO_ERRORS)
